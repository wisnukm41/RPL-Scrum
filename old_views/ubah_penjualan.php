<?php 
  ...
  $data = getDetailPenjualanObat($_GET['id']);
  if($data->num_rows < 1){
...
<head>
  ...
  <title>Ubah Penjualan || Apotek</title>
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Ubah Penjualan</h1>
          </div>

          <div class="card shadow mb-4 border-left-primary">
            <div class="card-header py-3 d-flex justify-content-between">
              <h6 class="m-0 font-weight-bold text-primary">Ubah Data Penjualan</h6>
              <a href="index.php" class="btn btn-warning">Kembali</a>
            </div>
            <div class="card-body">
            <form action="./action/update_sells.php" method="post">
                            ...
                            <tr>
                              ...
                              <select class="form-control select" name='id_obat[]'>
                                <?php 
                                  ...
                                  <option value="<?= $row->id ?>" data-harga="<?= $row->harga ?>" <?= $row->id === $sold->id ? "selected": ''  ?>><?= $row->nama ?> / <?= $row->jumlah ?> / Rp. <?= number_format($row->harga) ?></option>
                                <?php endwhile; ?>
                              </select>
                              </td>
                              <td>
                              ...
                              <td class='price'>Rp. <span><?= number_format($sold->total) ?></span></td>
                              <input type='hidden' class='hidden_price' name='price[]' value="<?= $sold->total ?>"/>
                              <td>
                                <?php
                                  if($total != 0): ?>
